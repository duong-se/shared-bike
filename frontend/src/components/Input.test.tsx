...
describe('Input', () => {
  ...
  it('should render and use fallback info', () => {
    const mockProps = {
      label: 'mockLabel',
      id: 'mockId',
      name: 'mockName',
    ...
    expect(Object.assign({},
      ...Array.from(input.attributes, ({ name, value }) => ({ [name]: value }))
    )).toStrictEqual({'aria-label': 'mockLabel', 'class': 'input input-bordered w-full input-error mockClassName', 'id': 'mockId', 'name': 'mockName', 'placeholder': '', 'type': 'text', 'value': 'mockValue'})
    const error = screen.getByText(mockProps.error)
    expect(error).toBeInTheDocument()
  })
